...
void asn1::EndPointDescriptor::set_id(){
	if(_id == NULL) _id = (Octet_string*)create_node(1);
}
...
asn1::Body::Body(){
	...
	// general
	_general = NULL;
	_general = new GeneralMessage();
	ASN1Node* _general_wrapper = new ASN1Node();
	_general_wrapper->tlv = new TLVNode();
	_general_wrapper->tlv->is_explicit = true;
	_general_wrapper->tlv->tag_class = CONTEXT_SPECIFIC;
	_general_wrapper->tlv->tag_value = 5;
	_general_wrapper->children.push_back(_general);
	children.push_back(_general_wrapper);
