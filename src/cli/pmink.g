...
typeItem : type=CSTRING ptrn=REGEX -> ^(TYPE_ITEM $type $ptrn)
;
...
configBlock : id+=IDENTIFIER id+=STAR*(L_BRACKET notify=notifiedDaemon R_BRACKET)* type=itemType desc=itemDesc* L_BRACE (cb+=configDef)* R_BRACE -> ^(CONFIG_BLOCK ^(ITEM_ID $id*) ^(ITEM_DESC $desc?) ^(NOTIFIED_DAEMON $notify?) ^(ITEM_TYPE $type) $cb*)
;


configItem : id=IDENTIFIER type=itemType desc=itemDesc* -> ^(CONFIG_ITEM ^(ITEM_ID $id) ^(ITEM_TYPE $type) ^(ITEM_DESC $desc?))
;
